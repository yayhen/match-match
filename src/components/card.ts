...
class Card {
  ...
  makeKardImageVisible(): void {
    ...
    const { imageWidth } = packList[this.packNumber];
    const { imageHight } = packList[this.packNumber];
    const { imagesInRow } = packList[this.packNumber];
    canvas.width = 100;
    canvas.height = 115;
    ...
    const cardToReplace = document.getElementById(this.cardId);
  ...
  cardClickHandler(): void {
    ...
    if (this.openedCard === false) {
      ...
      const cardOpenPosition: { x: number, y: number } = { x: NaN, y: NaN };
