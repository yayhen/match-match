...
class CardsContainer {
  ...
  addCardsToContainer() {
    const game = new Game({ x: App.gameSettings.getDifficunity(), y: App.gameSettings.getDifficunity() });
  ...
  public refresh(game: Game) {
    ...
    if (winGameCheck) {
      this.timer.pause();
      const modalCongradulation = new Congradulation(game.calculateScore(this.timer.seconds));
